...
scraper
    .get('https://www.tuttitalia.it/scuole/calendario-scolastico-2019-2020/veneto/')
    .then(function(tableData) {
        tableData = tableData.slice(1, tableData.length - 1)

        for (let idx = 0; idx < tableData.length; idx++) { // .slice() per togliere il primo lelemnto che è null & l'ultimo che e di una altra tabella nella pagina 
        ...
        date.push(date_ob.getMonth() > 6 ? `${date_ob.getFullYear() + 1}-05-21` : `${date_ob.getFullYear()}-05-21`)

        date.sort();

        console.log(date.slice(1, date.length - 2)) // .slice() per torgliere il giorno di inizio scuola e i due giorni di fine scuola (superiore & infanzia)
